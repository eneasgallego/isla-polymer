Polymer({
    ...
    mostrarOrden: function(orden, columna, desc) {
        var obj = orden.base.buscar('campo', columna.campo);
        return obj && ((obj.ascendente && !desc) || (!(obj.ascendente) && desc));
    },
    mostrarTextoColumna: function(columna) {
        return columna.texto ? columna.texto : columna.campo;
    },
    onQuitarOrden: function(e) {
        e.stopPropagation();
        var model = this.$.repeatHeader.modelForElement(e.target);
        var col = model.item;
        var campo = col.campo;
        var ordenItem = this.orden.buscar('campo', campo);
        if (ordenItem) {
            this.splice('orden', this.orden.indexOf(ordenItem), 1);
        }
    },
    onSort: function(e) {
        var campo = e.model.item.campo;
        var ordenItem = this.orden.buscar('campo', campo);
        if (ordenItem) {
            this.splice('orden', this.orden.indexOf(ordenItem), 1);
        } else {
